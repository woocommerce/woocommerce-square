...
cli()
{
	docker run -it --rm --user xfs --volumes-from woocommerce_square_wordpress --network container:woocommerce_square_wordpress wordpress:cli "$@" > /dev/null
}
...
cli wp core install --path=/var/www/html --url=localhost:8082 --title="WooCommerce Square Dev" --admin_name=admin --admin_password=admin --admin_email=admin@example.com --skip-email
...
echo "Activating the WooCommerce Square plugin..."
cli wp plugin activate woocommerce-square
