<?php
/**
 * Tests for WooCommerce\Square\Internal\Abilities\Abilities_Registrar.
 *
 * @package WooCommerce\Square
 */

namespace WooCommerce\Square\Tests\Unit\Internal\Abilities;
 ...
 * and WC 10.9 loader gates, plus filter wiring, append_classes round-trip,
 * the capability helper, and init() idempotency.
 */
class Abilities_Registrar_Test extends WP_UnitTestCase {

	const LOADER_FILTER  = 'woocommerce_ability_definition_classes';
	const FEATURE_FILTER = 'woocommerce_square_abilities_enabled';
	...
	public function test_init_is_no_op_when_feature_flag_disabled() {
		...
		$this->assertFalse(
			has_filter( self::LOADER_FILTER, array( Abilities_Registrar::class, 'append_classes' ) ),
	...
	public function test_can_manage_woocommerce_square_matches_manage_woocommerce_capability() {
	...
	public function test_init_is_idempotent_when_feature_flag_enabled() {
		...
		$this->assertSame(
			1,
			self::count_filter_callbacks( self::LOADER_FILTER, array( Abilities_Registrar::class, 'append_classes' ) ),
	 ...
	 *
	 * has_filter() returns the priority of the first match or false; it cannot
	 * tell us "the same callback is hooked twice." Walk the global $wp_filter
	 * structure directly so the idempotency assertion is load-bearing.
	 */
