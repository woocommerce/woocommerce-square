	...
	for ( const isBlock of isBlockCheckout ) {
		...
		for ( const isSCA of [ true, false ] ) {
			...
			test(
				title +
					subTitle +
					'[Charge upon release] Square Credit Card should work with Pre-Orders',
				async ( { page } ) => {
					if ( ! isBlock ) {
						// Skipping block checkout tests due to multiple iframe issue. ToDo: handle this later.
						test.skip();
					}
					...
					await expect(
						await orderStatus.evaluate( ( el ) => el.value )
					).toBe( 'wc-pre-ordered' );
					...
					await expect(
						page
							.locator(
								'#woocommerce-order-notes ul.order_notes li',
								{
									hasText:
										'Pre-Order Release Payment Approved:',
								}
							)
							.first()
					).toBeVisible();
			...
			test(
				title +
					subTitle +
					'[Upfront Charge] Square Credit Card should work with Pre-Orders',
				async ( { page } ) => {
					const productId = await createPreOrderProduct( page, {
