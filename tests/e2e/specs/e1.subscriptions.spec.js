...
test.describe('Subscriptions Tests', () => {
	...
	test('Customer can sign up to subscription using Saved CreditCard', async ({
		page,
	}) => {
		const isBlock = true;
		await page.goto( '/product/simple-subscription-product' );
	...
	test('Customer can early renew the subscription using Square CreditCard', async ({
		page,
	}) => {
		const isBlock = true;
	...
	test('Customer can change payment method of the subscription', async ({
		page,
	}) => {
		const isBlock = false;
		await page.goto( '/my-account/subscriptions/' );
