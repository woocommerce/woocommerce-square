...
- Gateway IDs and their settings keys - `square_credit_card`, `square_cash_app_pay`, `gift_cards_pay` - persisted in every order's `_payment_method` and in the `woocommerce_square_*_settings` option names, so changing one orphans merchant settings and detaches historical orders from the gateway that processed them.
