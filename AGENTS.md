...
**Overridable classes are contracts too, including which internal methods get called.** The gateway hierarchy exists to be subclassed - extensions override individual methods of `Payment_Gateway`, `Payment_Gateway_Direct`, `Gateway`, and `Cash_App_Pay_Gateway` - so the public and protected methods of any non-`final` class are contracts: changing their signatures or removing them breaks subclasses. Adding a fast path or skip that avoids calling an overridable method silently disables those overrides even though no signature changed: the extension's code simply stops running. When optimizing such a class, ensure overridable methods are still invoked on every code path, or treat the change as breaking.

**Registered script and style handles are public contracts.** Third-party code enqueues this plugin's handles, lists them as dependencies, or dequeues them by name - including handles that were only ever registered incidentally. Renaming or removing a handle breaks those consumers. To rename with a compatibility window, register the legacy handle as an alias that depends on the new handle (the same pattern WordPress core uses for `jquery` → `jquery-core`); do not register the same file under both handles, or pages with mixed consumers will load it twice.

**Database migrations must be one-shot and reversible one version back.** The `Lifecycle` upgrade path is keyed on the stored plugin version, so an `upgrade_to_*()` routine never re-runs once a site has passed it; keep any new schema or data update behind that keying. And each update must leave a rollback path: after reverting to the previous release, the old code runs against the migrated schema and must not fatal or corrupt.
